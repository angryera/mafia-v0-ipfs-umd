/**
 * getSlots - Fetches slot information for a city from MafiaMap contract
 * Map is 50x30 tiles (1500 slots total per city)
 */
...
const MAP_WIDTH = 50;
const MAP_HEIGHT = 30;
const BATCH_SIZE = 500; // Maximum slots per call

export interface ParsedSlotInfo {
  cityId: number;
  x: number;
  y: number;
  slotType: number; // 1: user slot, 2: protocol, 3: business, 4: family HQ, 5: raid spot
  slotSubType: number;
  variant: number;
  rarity: number; // 0: normal, 1: upper class, 2: elite, 3: strategic
  isOwned: boolean;
  isOperating: boolean;
...
type GetSlotsRaw = readonly [readonly RawSlot[], readonly `0x${string}`[]];
...
function parseSlots(
  ...
  return slots.map((slot, index) => ({
    ...
    owner: (owners[index] ?? '0x0000000000000000000000000000000000000000') as `0x${string}`,
  }));
}
...
export async function getSlots(
  ...
  const address = CONTRACTS.MafiaMap.addresses[chain];
  const abi = CONTRACTS.MafiaMap.abi;
