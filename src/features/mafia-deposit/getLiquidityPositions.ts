...
function parseLiquidityPosition(raw: RawLiquidityPosition): LiquidityPosition {
  return {
    ...
    initialCashAmount: raw.initialCashAmount ?? 0n,
    cashAmount: raw.cashAmount ?? 0n,
    cashPerMafia: raw.cashPerMafia ?? 0n,
    mafiaEarned: raw.mafiaEarned ?? 0n,
    mafiaWithdrawn: raw.mafiaWithdrawn ?? 0n,
    active: Boolean(raw.active),
  };
}

/**
 * Reads cash swap liquidity positions (in-game cash liquidity) from MafiaDeposit.
 *
 * - By default, fetches all listings.
 * - If `startIndex`/`length` are provided, reads only that range.
 * - If the deployed contract doesn't have `getLiquidityPositions`, this falls back to
 *   calling `liquidityPositions(index)` repeatedly until out-of-bounds.
 */
export async function getLiquidityPositions(options: {
  chain: ChainName;
  address?: `0x${string}`;
  startIndex?: number;
  length?: number;
  ...
  const address = (options.address ?? CONTRACTS.MafiaDeposit.addresses[chain]) as `0x${string}`;
  const client = getClient(chain);
  const abi = mafiaDepositAbi as unknown as Abi;
  const hasCustomRange = options.startIndex !== undefined || options.length !== undefined;
  const requestedStartIndex = Math.max(0, Math.floor(options.startIndex ?? 0));
  const requestedLength = Math.max(0, Math.floor(options.length ?? 0));
  const PAGE_SIZE = 500;
